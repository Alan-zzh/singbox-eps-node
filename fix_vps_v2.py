...
def main():
    ...
    # 1. 生成 Reality 密钥并写入 .env
    print("[1] 生成 Reality 密钥...")
    ...
    if private_key and public_key:
        # 使用 sed 写入 .env
        run(ssh, "sed -i 's/^REALITY_PRIVATE_KEY=.*/REALITY_PRIVATE_KEY=" + private_key + "/' /root/singbox-eps-node/.env")
        run(ssh, "sed -i 's/^REALITY_PUBLIC_KEY=.*/REALITY_PUBLIC_KEY=" + public_key + "/' /root/singbox-eps-node/.env")
        print("  ✅ Reality 密钥已写入\n")

    # 2. 修复 config_generator.py 路径
    print("[2] 修复 config_generator.py 路径...")
