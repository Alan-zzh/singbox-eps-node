...
## 当前版本
**v1.0.67** (Singbox已安装时交互选择卸载重装/保留+版本号更新)
...
**问题**: 安装脚本检测到singbox已安装时直接跳过，用户无法选择卸载重装
**修复**: 检测到已安装时提供交互式选择

**交互流程**：
```
Singbox 已安装，请选择操作：
  1) 卸载重装（删除当前版本，重新下载安装）
  2) 保留当前版本（默认，直接继续）

请输入选择 [1/2]（默认2）:
```

- 选1：停止singbox服务 → 删除/usr/local/bin/singbox → 重新下载安装
- 选2（默认）：保留当前版本，直接继续后续步骤
- 直接回车=选2（保留）
