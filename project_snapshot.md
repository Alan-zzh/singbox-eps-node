...
**版本**: v4.15.25 | **更新**: 2026-07-28（HKBEIYONG 备用直连生产闭环）
...
| 认证 SOCKS5 入站 | 按各机 `.env` | ✅ HK2/HKBEIYONG `:1080` 已外部验证 |
...
- HKBEIYONG 一键安装最终 `RC=0`；`deploy.py --verify --server HKBEIYONG` 为 9 PASS/1 合理 SKIP。
- HKBEIYONG 公网 Base64/Clash/sing-box/info 均 HTTP 200，三种配置各 4 个 `HKBEIYONG-*` 直连节点；证书 issuer=Let's Encrypt YE1、SAN 匹配、系统 CA 返回 0。
- 外部 TCP 443/2087/2096/1080/Trojan 随机端口可达；认证 SOCKS5 实际出口为 `47.242.36.160`；生产 `health_check.sh` 返回 0 且保持 `singbox-cdn` disabled。
- `python -m pytest -q`：`55 passed, 1 skipped`；`python tests/full_audit.py --server HKBEIYONG`：`ALL OK`。全量公网审计中 JP/HK2/HKBEIYONG 通过；HK1 因整机所有端口超时使全局结果为 `SOME ISSUES FOUND`，已明确归为既有 HK1 主机离线。
