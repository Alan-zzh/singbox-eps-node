...
# 1. Get full page and look for ALL script sources
...
code, out, err = run("curl -sk 'https://static-api.urlce.com/public/js/header.js' | grep -iE 'api|fetch|ajax|url|endpoint|data|get|post' | head -20")
print(out[:1500])

# 3. Look for other JS files on the page
print("\n[3] Check for other JS files on uouin.com")
code, out, err = run("curl -sk 'https://api.uouin.com/cloudflare.html' | grep -oP 'src=[\"\\'](/[^\"\\']+\\.js[^\"\\']*)[\"\\']' | sort -u")
print(out)
...
code, out, err = run("curl -sk 'https://api.uouin.com/cloudflare.html' | grep -oP 'var\\s+\\w+\\s*=\\s*\\{[^}]+\\}' | head -10")
print(out[:500])

# 8. Check for WebSocket or SSE connections
print("\n[8] Check for WebSocket/SSE")
code, out, err = run("curl -sk 'https://api.uouin.com/cloudflare.html' | grep -iE 'websocket|ws://|wss://|eventsource|sse' | head -5")
print(out[:500])
...
code, out, err = run("curl -sk 'https://api.uouin.com/cloudflare.html' | grep -oP '<script[^>]*>[^<]*\\{[^}]+\\}[^<]*</script>' | head -5")
print(out[:1000])

# 10. Try to find the API by checking network requests pattern
print("\n[10] Check for common API patterns in the JS files")
code, out, err = run("curl -sk 'https://static-api.urlce.com/public/js/header.js' | grep -oP '(?:api|fetch|ajax|get|post|request)\\s*\\(\\s*[\"\\'][^\"\\']+[\"\\']' | head -10")
print(out[:500])
