...
def generate_all_links():
    ...
    params = {
        ...
        'sni': REALITY_SNI, 'fp': 'chrome', 'pbk': REALITY_PUBLIC_KEY, 'sid': REALITY_SHORT_ID,
        'spx': '', 'dest': REALITY_DEST, 'headerType': 'none'
    }
...
def create_app():
    ...
    def home():
        ...
        html = f"""
        <html><head><title>Singbox订阅服务</title>
        <style>body {{ font-family: Arial, sans-serif; max-width: 600px; margin: 50px auto; padding: 20px; }}</style>
        </head><body>
        <h1>Singbox 订阅服务</h1>
        <p>当月流量: {traffic_display}</p>
        <p>Base64订阅: https://{CF_DOMAIN or SERVER_IP}:{SUB_PORT}/sub/{COUNTRY_CODE}</p>
        <p>sing-box JSON: https://{CF_DOMAIN or SERVER_IP}:{SUB_PORT}/singbox/{COUNTRY_CODE}</p>
        </body></html>
        """
