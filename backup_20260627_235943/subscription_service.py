...
try:
    ...
    _hk_direct_fallback = (os.getenv('COUNTRY_CODE', 'US') or '').upper() == 'HK'
...
if 'HK_DIRECT_MODE' not in dir():
    HK_DIRECT_MODE = (COUNTRY_CODE or '').upper() == 'HK'

# [Codex] v4.15.0 客户端能力矩阵（修复 v2rayN 订阅解析失败问题）
# 决定 /sub 端点返回哪些节点
#   full     = 完整节点（含 anyTLS），仅明确支持 anytls:// URI scheme 的客户端
#              - Clash Meta (mihomo) 系、sing-box 系、NekoBox/NekoRay
#   xray     = Xray 兼容节点（不含 anyTLS），只返回标准 vless:// 和 trojan:// 链接
#              - v2rayN/v2rayNG/Shadowrocket/Quantumult/Surge/Loon 等
#   standard = 同 xray（兼容旧参数），不含 anyTLS
#   unknown  = 按 xray 处理（安全默认，避免非标准 URI 导致解析失败）
#
# 【Bug #? 根因】：anytls:// 不是标准代理分享协议 scheme，Xray-core 客户端不认识，
# 之前错误地给 v2rayN 返回 anytls:// 链接，会导致部分版本 v2rayN 整个订阅解析失败。
CLIENT_CAPABILITIES = {
    # Clash 系（mihomo 内核支持 Reality/gRPC/anyTLS，能正确解析 anytls://）
    'clash-meta': 'full',
    'clash-verge': 'full',
    'clash verge': 'full',
    ...
    'mihomo-party': 'full',
    'stash': 'full',
    # sing-box 原生客户端（支持所有协议，含 anyTLS）
    'sing-box': 'full',
    'singbox': 'full',
    'nekobox': 'full',
    'nekoray': 'full',
    # v4.15.0: Xray-core 系客户端不支持 anytls:// scheme，必须排除
    # v2rayN/v2rayNG/Shadowrocket/Quantumult/Surge/Loon 等均使用标准 vless/trojan URI
    'v2rayn': 'xray',
    'v2rayng': 'xray',
    'v2ray ng': 'xray',
    'v2box': 'xray',
...
def generate_all_links(capability='full'):
    ...
    - DIRECT_MODE_ENABLED（直连精简模式）：
      * full: 4节点（VLESS-Reality/VLESS-gRPC/Trojan-TCP/anyTLS）
      * xray: 3节点（VLESS-Reality/VLESS-gRPC/Trojan-TCP，不含 anyTLS）
    - CDN_MODE_ENABLED（CDN全量模式）：
      * full: 6节点（加上 VLESS-WS-CDN/Trojan-WS-CDN/anyTLS）
      * xray: 5节点（加上 VLESS-WS-CDN/Trojan-WS-CDN，不含 anyTLS）
    - capability='xray'：Xray-core 兼容客户端（v2rayN等），跳过 anytls:// 非标准URI
    - capability='standard'：等同 xray（兼容旧参数）
    """
    include_anytls = (capability == 'full')
    links = []
    ...
    # 6. anyTLS (直连) - 仅 full 能力客户端输出（anytls:// 非标准URI，Xray系客户端不认识）
    if include_anytls:
...
def create_app():
    ...
    def home():
        ...
        node_count_full = 4 if DIRECT_MODE_ENABLED else 6
        node_count_xray = 3 if DIRECT_MODE_ENABLED else 5
        ...
        if not DIRECT_MODE_ENABLED:
            sub_info_cdn = '<p class="info">- VLESS-WS-CDN / Trojan-WS-CDN：走 sub-* 直连源站，绕过CF DDoS L7</p>'
        ...
        <body>
            ...
            <p class="info">当前模式：{mode_desc}（Clash/sing-box 客户端 {node_count_full} 节点 | v2rayN/Xray 客户端 {node_count_xray} 节点）</p>
            ...
            <div class="sub-box">
                ...
                <p class="info">- Clash Meta / sing-box / NekoBox / NekoRay：{node_count_full} 节点（含 anyTLS）</p>
                <p class="info">- v2rayN / v2rayNG / Shadowrocket / Surge / Quantumult X：{node_count_xray} 节点（不含 anyTLS，标准 URI 兼容）</p>
                <p class="info">- Shadowrocket CONNECT/HTTP 测速更接近真实可用性；ICMP 仅作裸线路参考</p>
                <p class="info">- 💡 强制指定：?client=clash 获全量节点 | ?client=v2rayn 获 Xray 兼容节点</p>
                {sub_info_cdn}
            </div>
            <div class="sub-box">
                ...
                <p class="info">- 节点数：{node_count_full}（{mode_label}，含 anyTLS）</p>
            </div>
            <div class="sub-box">
    ...
    def get_subscription():
        ...
        【v4.15.0 客户端能力适配（修复v2rayN订阅问题）】:
        - 根据 User-Agent 自动判断客户端能力
        - Clash Meta/mihomo/sing-box/NekoBox/NekoRay（full）→ 含 anyTLS 节点
        - v2rayN/v2rayNG/Shadowrocket/Surge/Quantumult X/Loon（xray）→ 标准URI，不含 anyTLS
