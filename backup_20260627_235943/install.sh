...
setup_iptables_traffic_counter() {
    ...
    netfilter-persistent save 2>/dev/null || iptables-save > /etc/iptables/rules.v4 2>/dev/null || true
    if [ "$DEPLOY_MODE_IPT" = "direct" ]; then
        log_info "iptables流量计数器已配置（纯直连模式：端口443/2087/2096/$VLESS_GRPC_PORT/$TROJAN_TCP_PORT/$TUIC_PORT_IPT）"
    else
        log_info "iptables流量计数器已配置（CDN混合模式：端口443/8443/2083/2087/2096/$VLESS_GRPC_PORT/$TROJAN_TCP_PORT/$TUIC_PORT_IPT）"
    fi
}
...
print_summary() {
    ...
    if [ "$DEPLOY_MODE_SUMMARY" = "direct" ]; then
        echo "📡 纯直连模式，无CDN依赖"
        echo "  节点: 4个（VLESS-Reality/gRPC + Trojan-TCP + anyTLS + VLESS-TCP-Reality）"
    else
        echo "🌐 CDN优选IP（4级降级保障）:"
