...
# v4.14.1: HK1 香港阿里云节点特殊模式 - 全部直连，无 CDN 节点
# 其他节点（JP/SG）保持原有 CDN(sub-* gray cloud) 模式不变
# v4.15.0: 此标志作为 legacy 向后兼容依据，新部署统一使用 DEPLOY_MODE 控制
HK_DIRECT_MODE = (os.getenv('COUNTRY_CODE', '') or _load_env_value('COUNTRY_CODE', '') or '').upper() == 'HK'

# v4.15.0: 部署模式 dual-stack 支持
# 'cdn'    = CDN混合模式（6节点：4直连+2WS-CDN，启动singbox-cdn服务）
# 'direct' = 纯直连模式（4节点：去掉WS-CDN和CDN监控，极简无CF依赖）
# 向后兼容策略：
#   - 如果 .env 中显式设置了 DEPLOY_MODE → 遵循显式设置
#   - 如果未设置 DEPLOY_MODE（旧部署升级）：
#     * HK 节点（COUNTRY_CODE=HK）→ 默认 direct（保持 v4.14.1 现有行为）
#     * 其他节点（JP/SG等）→ 默认 cdn（保持现有行为）
